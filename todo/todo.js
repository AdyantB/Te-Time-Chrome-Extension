...
function displayTasks() {
  ...
  todoList.todoTasks.forEach((task, i) => {
    ...
    listItem.append(listContent, removeButton);
...
const startSr = () => {
    ...
    r.start();
    r.addEventListener("result", (res) => {
        if (res.results[0].isFinal && isListening) {
            stopSr();
            addTask(res.results[0][0].transcript, false, todoList.todoTasks[todoList.todoTasks.length - 1], todoList);
            displayTasks();
        }
